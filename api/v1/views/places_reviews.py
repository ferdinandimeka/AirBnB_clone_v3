...
def reviewAll(id):
    ...
    lists = []
    s = storage.alists('Place').values()
    ss = storage.alists('Review').values()
    for v in s:
        if v.id == id:
            for vv in ss:
                if vv.place_id == id:
                    lists.append(vv.to_dict())
    if not lists:
        return abort(404)
...
def reviewId(id):
    ...
    lists = []
    s = storage.alists('Review').values()
    for v in s:
        if v.id == id:
...
def reviewPost(id):
    ...
    x = request.get_json()
    if "User." + x["user_id"] not in storage.alists("User"):
        abort(404)
    x['place_id'] = str(id)
    if "user_id" not in x:
